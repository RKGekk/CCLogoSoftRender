...
namespace BasicRender.Engine {
    ...
    public class Renderer {
        ...
        public void printPolyZ(RPolygon pIn, Vec2f pixelLeftTop, Vec2f pixelRightBottom) {

            RPolygon polygon = pIn;
            ...
            if (polygon.isOffScreen(pixelLeftTop, pixelRightBottom))
                return;
            ...
            for (int x = leftTop.x; x < bottomRight.x; x++) {
                for (int y = leftTop.y; y < bottomRight.y; y++) {
                    ...
                    if ((w.x >= 0.0f && w.y >= 0.0f && w.z >= 0.0f) || (w.x <= 0.0f && w.y <= 0.0f && w.z <= 0.0f)) {
                        ...
                        Vec2f UV = new Vec2f(
                            (polygon.vertex0.textureCoordinates.x * polygon.vertex0.z * w.x + polygon.vertex1.textureCoordinates.x * polygon.vertex1.z * w.y + polygon.vertex2.textureCoordinates.x * polygon.vertex2.z * w.z) * z,
                            (polygon.vertex0.textureCoordinates.y * polygon.vertex0.z * w.x + polygon.vertex1.textureCoordinates.y * polygon.vertex1.z * w.y + polygon.vertex2.textureCoordinates.y * polygon.vertex2.z * w.z) * z
                        );
        ...
        public void renderPolySolidColorZ(bool isWireframe) {
            ...
            renderObject.model = Mat4f.RotationXMatrix(GeneralVariables.M_PI * 0.5f) * Mat4f.RotationZMatrix(GeneralVariables.M_PI * 0.5f) * Mat4f.RotationYMatrix(_angle);
            renderObject.world = new Mat4f(
                new Vec4f(0.5f, 0.0f, 0.0f, 0.0f),
                new Vec4f(0.0f, 0.5f, 0.0f, 0.0f),
                new Vec4f(0.0f, 0.0f, 0.5f, 0.0f),
                new Vec4f(0.0f, 0.0f, 6.0f, 1.0f)
            );
            renderObject.view = camera.GetViewMatrix();
            renderObject.proj = camera.GetProjMatrix();

            renderObject.calculateAll(0.1f, 15.0f, pixelWidthf, pixelHeightf, true, false);

            Parallel.For(0, 20, i => {
                foreach (RPolygon polygon in renderObject.polygons_raster) {
                    printPolyZ(
                        polygon,
                        new Vec2f(this.pixelLeftTop.x, i * 10),
                        new Vec2f(this.pixelRightBottom.x, i * 10 + 9)
                    );
                }
            });
